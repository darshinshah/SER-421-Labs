...
try{
	...
	var databaseread=fs.readFileSync('database.txt','utf-8').split('\n')
	for(jsonstring of databaseread){
		if(jsonstring=='null'){
			continue;
		}
		database.push(JSON.parse(jsonstring))
	}
}
catch(e){
	console.log("")
...
http.createServer(function (req, res) {
	...
	if (req.method == "GET") {
		if(urlObj.pathname=="/my_groceries"){
			...
			if(!urlObj.searchParams.get('aisle') && !urlObj.searchParams.get('custom') && Array.from(urlObj.searchParams).length >1 ){
				res.writeHead(400)
		...
		req.on('end',function(){
			...
			var stringdatabase=null
			for(let elementof of database){
				stringdatabase+='\n'+JSON.stringify(elementof)
			}

			fs.writeFile("database.txt",stringdatabase,function(err){
				if(err){
					console.log(err);
				}
			})
